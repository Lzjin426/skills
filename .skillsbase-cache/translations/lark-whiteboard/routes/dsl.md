...
```
步骤 1: 路由 & 读取知识
  - 阅读对应场景指南 — 了解结构特征和布局策略
  - 确定布局策略（见下方快速判断）和构建方式
  - 阅读 references/ 核心模块 — 语法、布局、配色、排版、连线
...
步骤 2: 生成完整 DSL（含颜色）
  ...
  注意：部分图形（鱼骨/飞轮/柱状/折线等）要按场景指南的脚本模板写 CommonJS 脚本生成 JSON：
...
> **构建方式是强约束**：当场景指南要求"脚本生成"时，必须先写脚本（`.cjs`，CommonJS）并用 `node` 执行来产出 JSON 文件。
