---
name: "doc"
description: "当任务涉及读取、创建或编辑 `.docx` 文档，尤其是需要保持格式或版式保真度时使用；推荐优先使用 `python-docx` 配合内置的 `scripts/render_docx.py` 进行视觉检查。"
---
...
4. 如果无法进行视觉审阅，使用 `python-docx` 提取文本作为备选方案，并指出版式风险。
5. 保持中间输出文件有序，并在最终确认后清理。

## 临时文件与输出约定
- 使用 `tmp/docs/` 存放中间文件；完成后删除。
- 在本仓库中工作时，将最终输出文件写入 `output/doc/`。
- 保持文件名稳定且具有描述性。
