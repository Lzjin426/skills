...
实现有目的、高性能的动画，以增强用户体验而不致使用户感到负担。关注关键场景：首屏介绍、悬停反馈、内容展现和导航过渡。
...
2. **确定触发方式** – 加载、滚动（交叉观察）、悬停、点击
3. **估算工作量** – 低（仅 CSS）、中（需要钩子）、高（需要库）
4. **提出分阶段实施计划** – 先实现快速见效的部分
