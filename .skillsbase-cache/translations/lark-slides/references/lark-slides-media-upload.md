...
将本地图片上传到指定演示文稿的 drive 媒体库，返回 `file_token`。**返回的 token 可作为 `<img src="...">` 的值插入 slide XML 以显示图片。**
...
- **`file_token`**：将其写进 `<img src="...">`
- **`file_name` / `size`**：上传文件的元信息
...
### 新建带图 PPT（推荐使用 `+create --slides` 的 `@` 占位符，一步到位）
...
> ⚠️ slides XML API 没有元素级编辑接口（见 SKILL.md 核心规则 7）—— 无法“往现有 slide 上贴一张图”，只能**把整页替换**：读取原 slide → 添加 `<img>` → 原位插入新页 → 删除旧页。
...
# 2) 读取整份 PPT，摘出目标 slide 的完整 XML（保留所有 shape/line/img 原样）
lark-cli slides xml_presentations get --as user \
  --params "{\"xml_presentation_id\":\"$PRES_ID\"}" \
  | jq -r '.data.xml_presentation.content'

# 3) 在 agent 侧拼接新 slide XML：原有所有元素原样保留 + 新增一个 <img>
#    关键：先查看原 <data> 里现有元素的 topLeftX/Y/width/height，将 <img> 放到空白区域
...
1. **先 create 后 delete** —— 顺序反了且 create 失败会丢失页面
2. **原 slide 的所有元素必须原样搬过来** —— 只写新 `<img>` 会把原页标题/正文/形状全部删除
3. **`before_slide_id=<旧 slide_id>` 必传** —— 不传新页会追加到末尾，打乱页面顺序
4. **`<img>` 坐标需避开现有元素** —— 先读取现有元素 bbox 挑选空白区域；空间不够就缩小/挪动现有元素后再放图
5. **`<img>` 的 `width:height` 仍需对齐原图比例** —— 比例不一致会被裁剪，参见 [xml-schema-quick-ref.md](xml-schema-quick-ref.md) `<img>` 说明
6. **`slide_id` 会变化** —— 新页有新 ID，外部有深链依赖的需要告知用户
...
**不要尝试使用 `slides_image`、`slide_image` 等 parent_type**——后端会返回 1061001 / 1061002 错误。这是 slides 的特殊约定。

## 常见错误

| 错误码 | 含义 | 解决方案 |
|--------|------|----------|
| 1061002 | params error / 不支持的 parent_type | 不要用原生 API 自己拼 parent_type；使用 `+media-upload` 即可 |
