...
1. 建议先用 `+view-get-filter` 拉取当前配置，再做最小化修改。

## 注意事项

- ⚠️ 这是写入操作，执行前必须确认。
- ⚠️ 条件必须使用元组格式，不要再写旧的 `{"field_name":...,"operator":...}` 对象风格。
- ⚠️ `empty` / `non_empty` 不要硬塞 value；`select` / `user` / `link` 也不要直接写单值。
- ⚠️ 日期值要保留 `ExactDate(...)` 外壳，不要直接写裸日期字符串。

## 参考

- [lark-base-view.md](lark-base-view.md) — 视图索引页
- [lark-base-view-get-filter.md](lark-base-view-get-filter.md) — 读取筛选配置