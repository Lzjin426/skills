...
> 生成或改写飞书电子表格公式时的参考规则。飞书不像 Excel 365 默认支持溢出（spill），普通公式对区域默认进行“投影”（只取当前行/列对应的单值），必须显式使用 `ARRAYFORMULA` 或原生数组函数才能实现逐项展开。
...
当行号/列号/偏移量本身是数组时，必须显式包裹 `ARRAYFORMULA`：
...
在 Excel 中，`SUBTOTAL`、`INDIRECT`、`OFFSET` 等函数在 `SUMPRODUCT` 内部会隐式逐行求值，但飞书不会。需要使用 MAP 显式遍历：
...
同类场景：当 `SUMIF/COUNTIF/SUMIFS` 的范围参数来自 `INDIRECT/OFFSET` 时，也需要使用 MAP。

## 多维结果降维

飞书公式结果只能是二维的，不能返回“区域的列表”。合并多个区域时：
...
| `=A1#`（溢出范围） | 改为明确的范围，或使用 `TAKE`/`DROP`/`ARRAY_CONSTRAIN` |
| `=SUM(Table1[Amount])`（结构化引用） | `=SUM(A2:A100)`（改为 A1 区域引用） |
| `{=A1:A10*B1:B10}`（CSE 花括号数组公式） | `=ARRAYFORMULA(A1:A10*B1:B10)` |
