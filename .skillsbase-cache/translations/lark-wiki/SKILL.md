...
> - 如果用户明确要求“以 bot 身份运行”，且目标是部门，必须停下说明 bot 路径无法完成，不要静默切换到 `--as user`。
...
- 部门场景使用 `member_type=opendepartmentid`：当前 CLI 没有快捷方式，需调用 `lark-cli api POST /open-apis/contact/v3/departments/search --as user --params '{"department_id_type":"open_department_id"}' --data '{"query":"<部门名>"}'` 获取 `open_department_id`。
- 只有在目标类型和身份都已确认可行后，才调用 `lark-cli wiki members create`。对于部门场景，这意味着必须是 `--as user`。

## 目标语义约束

- `我的文档库` / `My Document Library` / `我的知识库` / `个人知识库` / `my_library` 都应视为 **Wiki 个人知识库**，不是 Drive 根目录
...
快捷方式是对常用操作的高级封装（`lark-cli wiki +<verb> [flags]`）。有快捷方式的操作优先使用。

| 快捷方式 | 说明 |
|----------|------|
| [`+move`](references/lark-wiki-move.md) | 移动 Wiki 节点，或将 Drive 文档移入 Wiki |
| [`+node-create`](references/lark-wiki-node-create.md) | 创建 Wiki 节点，并自动解析空间信息 |
