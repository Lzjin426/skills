...
在 `:active` 状态添加 `transform: scale(0.97)` 实现即时按压反馈。
...
模糊值保持在 20px 以下——重度模糊性能开销大，尤其在 Safari 中。

## 弹出层与下拉菜单

从触发点缩放进入，而非从中心。默认的 `transform-origin: center` 不适用于弹出层。
...
从 `scale(0.88)` 开始，永远不要用 `scale(0)`。现实世界中没有任何事物是从无到有出现的。
...
首次出现前延迟（300–500ms）以防止意外触发。一旦某个工具提示已打开，后续工具提示应无动画立即打开。
...
使用移动缓动曲线。百分比 `translateY`/`translateX` 可适应任何抽屉高度。
...
**例外：模态框保持 `transform-origin: center`。** 它们代表应用级状态，不锚定于某个触发器。

使用 `@starting-style` 实现无需 JavaScript 的入场动画：
...
对消息提示使用 CSS 过渡（而非关键帧动画）——它们被频繁添加，关键帧动画在中断时会重新开始，而过渡动画能平滑地重定向。

## 列表与错开动画

保持错开延迟简短（每项 30–50ms）。总错开时间应控制在 300ms 以内。
