...
这能实现无缝的颜色过渡效果，而单独对 `color` 进行过渡动画永远无法达到这种效果。
...
在彩色遮罩层上使用 `clip-path: inset(0 100% 0 0)`。在 `:active` 状态下，以 `linear` 缓动函数在 2 秒内过渡到 `inset(0 0 0 0)`。释放时，以 200 毫秒的 `ease-out` 快速恢复。同时配合按钮的 `scale(0.97)` 效果提供按压反馈。
...
初始状态设为 `clip-path: inset(0 0 100% 0)`（从底部隐藏）。当元素进入视口时，动画过渡到 `inset(0 0 0 0)`。
